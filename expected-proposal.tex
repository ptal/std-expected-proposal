...
An object of type \cpp{E} is not convertible to an unexpected object of type \suppress{\cpp{expected<E,T>}}\update{\cpp{expected<T,E>}} since \cpp{E} and \cpp{T} can be of the same type. The proposed interface uses a special tag \cpp{unexpect} and a special non-member \cpp{make_unexpected} function to indicate an unexpected state for \suppress{\cpp{expected<E,T>}}\update{\cpp{expected<T,E>}}. It is used for construction and assignment. This might rise a couple of objections. First, this duplication is not strictly necessary because you can achieve the same effect by using the \cpp{unexpect} tagged forwarding constructor:

\begin{alltt}
\suppress{expected<int, string> exp1 = make_unexpected(1); }
\update{expected<string, int> exp1 = make_unexpected(1); }
\suppress{expected<int, string> exp2 = \{unexpect, 1\}; }
\update{expected<string, int> exp2 = \{unexpect, 1\}; }
...
\begin{alltt}
\suppress{expected<int, vector<int>> get1() \{\}}
\update{expected<vector<int>, int> get1() \{\}}
  return {unexpect, 1};
\}

\suppress{expected<int, vector<int>> get2() \{}
\update{expected<vector<int>, int> get2() \{}
  return make_unexpected(1);
\}

\suppress{expected<int, vector<int>> get3() \{}
\update{expected<vector<int>, int> get3() \{}
\suppress{  return expected<int, vector<int>>{unexpect, 1};}
\update{  return expected<vector<int>, int>{unexpect, 1};}
\}
\end{alltt}

The usage of \cpp{make_unexpected} is also a consequence of the adapted model for expected: a discriminated union of \cpp{T} and \cpp{unexpected_type<E>}. While \cpp{make_unexpected(E)} has been chosen because it clearly indicates that we are interested in creating an unexpected \suppress{\cpp{expected<E,T>}}\update{\cpp{expected<T,E>}} (of unspecified type \cpp{T}), it could be also used to make a ready future with a specific error, but this is outside the scope of this proposal. Note also that the definition of the result type of \cpp{make_unexpected} has an explicitly deleted default constructor. This is in order to enable the reset idiom \cpp{exp2 = \{\}} which would otherwise not work due to the ambiguity when deducing the right-hand side argument.
...
\suppress{bool has_error(expected<E,T> const&, E err) \{}
\update{bool has_error(expected<T,E> const&, E err) \{}
  if (e) return false;
...
This subclause describes class template \cpp{unexpected_type} that wraps objects intended as unexpected. This wrapped unexpected object is used to be implicitly convertible to other objects.
...
A program that necessitates the instantiation of template \cpp{unexpected_type} for a reference type or \cpp{void} is ill-formed.
...
This subclause describes class template expected that represents expected objects. An \suppress{\cpp{expected<E,T>}}\\
\update{\cpp{expected<T,E>}} object is an object that contains the storage for another object and manages the lifetime of this contained object T, alternatively it could contain the storage for another unexpected object E. The contained object may not be initialized after the expected object has been initialized, and may not be destroyed before the expected object has been destroyed. The initialization state of the contained object is tracked by the expected object.
...
A program that necessitates the instantiation of template \suppress{\cpp{expected<E,T>}}\update{\cpp{expected<T,E>}} with \cpp{T} for a reference type\, or for possibly cv-qualified types \cpp{in_place_t}, \cpp{unexpect_t} or \cpp{unexpected_type<E>} is ill-formed.

\wordingSubSec{Definitions}{expected.defs}
%%%%%%%%%%%%%%%%%%%%%%

An instance of \suppress{\cpp{expected<E,T>}}\update{\cpp{expected<T,E>}} is said to be valued if it contains an value of type \cpp{T}.
An instance of \suppress{\cpp{expected<E,T>}}\update{\cpp{expected<T,E>}} is said to be unexpected if it contains an object of type \cpp{E}.
...
\end{lstlisting}
\begin{alltt}
\suppress{      typedef expected<error_type, U> type;}
\update{      typedef expected<U, error_type> type;}
\end{alltt}
\begin{lstlisting}
    };
...
\begin{alltt}
    ...
    \suppress{expected<E,T> catch_exception(F&& f);}
    \update{expected<T,E> catch_exception(F&& f);}

    template <typename F>
      \suppress{expected<E, decltype(func(declval<T>()))> map(F&& func) ;}
      \update{expected<decltype(func(declval<T>())),E> map(F&& func) ;}
    template <typename F>
      'see below' bind(F&& func);
    template <typename F>
      \suppress{expected<E,T> catch_error(F&& f);}
      \update{expected<T,E> catch_error(F&& f);}
    template <typename F>
...
Valued instances of \suppress{\cpp{expected<E,T>}}\update{\cpp{expected<T,E>}} where \cpp{T} and \cpp{E} is of object type shall contain a value of type \cpp{T} or a value of type \cpp{E} within its own storage. This value is referred to as the contained or the unexpected value of the expected object. Implementations are not permitted to use additional storage, such as dynamic memory, to allocate its contained or unexpected value. The contained or unexpected value shall be allocated in a region of the \suppress{\cpp{expected<E,T>}}\update{\cpp{expected<T,E>}} storage suitably aligned for the type \cpp{T} and \cpp{E}.
\newline
...
% Destructor.
\begin{alltt}
\suppress{expected<E,T>::}~expected();
\end{alltt}
\begin{wordingPara}
\begin{wordingTextItem}{Effects}
...
\begin{alltt}
template <class U, class... Args>
void \suppress{expected<E,T>::}emplace(initializer_list<U> il, Args&&... args); 
\end{alltt}
\begin{wordingPara}
\begin{wordingTextItem}{Effects}
...
% swap operation.
\begin{alltt}
\suppress{void expected<E,T>::swap(expected<E,T>& rhs) noexcept(/*see below*/); }
\update{void swap(expected<T,E>& rhs) noexcept(/*see below*/); }
\end{alltt}
\begin{wordingPara}
...
\begin{alltt}
\suppress{    template <class E, class T>}
\suppress{    template expected<E,T> expected<E,T>::unwrap() &&;}
\update{    template <class T, class E>}
\update{    template expected<T,E> expected<T,E>::unwrap() &&;}
\end{alltt}
...
\begin{wordingTextItem}{Remarks}
The function shall not participate in overload resolution unless: \\
\suppress{\cpp{is_move_constructible<expected<E,T>>::value}}\suppress{\cpp{is_move_constructible<expected<T,E>>::value}} \\
\end{wordingTextItem}
...
expected<\suppress{E},T> expected<\suppress{E},T>::catch_exception(F&& func);
\end{alltt}
\begin{wordingPara}
\begin{wordingTextItem}{Effects}
...
\end{wordingTextItem}
\begin{wordingTextItem}{Returns}
if \cpp{has_exception<Ex>()} returns the result of the call continuation function \cpp{fuct} possibly wrapped on a \suppress{\cpp{expected<E,T>}}\update{\cpp{expected<T>}}, 
...
\update{  class expected<holder, E>}
  ...
  public:
    template <class T>
\suppress{    using type = expected<E,T>}
\update{    using type = expected<T,E>}
  };
\end{alltt}
...
\begin{alltt}
    template <typename Ex, typename F>
\suppress{    expected<E,void> catch_exception(F&& f);}
\update{    expected<void,E> catch_exception(F&& f);}

    template <typename F>
\suppress{      expected<E, decltype(func())> map(F&& func) ;}
\update{      expected<decltype(func()), E> map(F&& func) ;}
    template <typename F>
      'see below' bind(F&& func) ;
    template <typename F>
...
\begin{alltt}
template <class T, class E>
\suppress{void swap(expected<E,T>& x, expected<E,T>& y) noexcept(noexcept(x.swap(y))); }
\update{void swap(expected<T,E>& x, expected<T,E>& y) noexcept(noexcept(x.swap(y))); }
\end{alltt}
\begin{wordingPara}
\begin{wordingTextItem}{Effects}
calls \cpp{x.swap(y)}.
...
\begin{alltt}
template <class T>
\suppress{constexpr expected<exception_ptr, typename decay<T>::type> make_expected(T&& v);}
\update{constexpr expected<typename decay<T>::type> make_expected(T&& v);}
\end{alltt}
\begin{wordingPara}
\begin{wordingTextItem}{Returns}
\suppress{\cpp{expected<exception_ptr, typename decay<T>::type>(std::forward<T>(v))}} \\
\update{\cpp{expected<typename decay<T>::type>(std::forward<T>(v))}}.
\end{wordingTextItem}
\end{wordingPara}
...
\begin{alltt}
template <class T, class E>
\suppress{struct hash<expected<E,T>>; }
\update{struct hash<expected<T,E>>; }
\end{alltt}
\begin{wordingPara}
\begin{wordingTextItem}{Requires}
The template specialization \cpp{hash<T>} and \cpp{hash<E>} shall meet the requirements of class template \cpp{hash} (Z.X.Y). The template specialization \suppress{\cpp{hash<expected<E,T>>}}\update{\cpp{hash<expected<T,E>>}} shall meet the requirements of class template \cpp{hash}. For an object \cpp{e} of type \\
\suppress{\cpp{expected<E,T>}}\update{\cpp{expected<T,E>}}, \cpp{if bool(e)}, \suppress{\cpp{hash<expected<E,T>>()(e)}}\update{\cpp{hash<expected<T,E>>()(e)}} shall evaluate to the same value as \cpp{hash<T, E>()(*e);} otherwise it evaluates to an unspecified value if \cpp{E} is \cpp{exception_ptr}  or \cpp{hash<T, E>()(e.error());}.
\end{wordingTextItem}
\end{wordingPara}

\begin{alltt}
template <class E>
\suppress{struct hash<expected<E, void>>;}
\update{struct hash<expected<void, E>>;}
\end{alltt}
\begin{wordingPara}
\begin{wordingTextItem}{Requires}
\end{wordingTextItem}
