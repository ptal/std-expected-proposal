...
Unlike \cpp{optional<T>}, the goal of expected is not to provide a wrapper for types that are not default-constructible. Hence, the authors consider that \cpp{expected<T,E>} should have a default constructor if and only if \cpp{T} is default-constructible, and if so, default construct to \cpp{T\{\}}. If \cpp{T} is not default constructible, the user will be free to wrap it inside an optional (\cpp{expected<optional<T>, E>}).
