...
\begin{lstlisting}
expected<int> f1(int i, int j, int k)
{
...
The next use case shows how expected scales and compose. Given the functions \cpp{getInt} and \cpp{matchesString}, how to define a function that parses an int range ``<int>..<int>'' \cpp{getIntRange}?

Next follows the Standard IO facet style to get an int from a stream (only the iterators are shown):
...
In addition to manage with the Iterators, the function changes the error and the int value itself. Let start by defining the function as if the functions throw an exception when there is an error and make use of an hypothetic istream_range class that would encapsulate the iterators. 
...
Because the first behaviour leads to buggy application, we won't consider it in a first time.
...
Thanks to Fernando Cacciola, Andrzej KrzemieÅ„ski and every one that has contributed to the wording and the rationalle of N3793 \cite{OptionalRev5}.
