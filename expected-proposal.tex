...
Clearly this is quite clean. However, when exceptions are not desirable, \cpp{expected<>} come to play. 
...
Line 1: obtains an \cpp{expected<int>} possibly with the parsed int. 
Line 2: the call to \cpp{next()} will return an \cpp{expect<int>} as the lambda returns an \cpp{expect<int>}. This is due to the fact that next unwraps the result so that \cpp{expect<expect<int>>} is unwrapped to \cpp{expect<int>}. 
If it is valid it calls to the lambda and unwraps the result. It it it invalid, it returns an \cpp{expect<int>} with the obtained error.
Line 3: returns an expected that combines first and the success of the call to \cpp{matchesString("..", s, e)}.
Line4:  the call to \cpp{next()} will return an \cpp{expect<pair<int,int>>} as the lambda returns an \cpp{expect<pair<int,int>>}. This is due to the fact that next unwraps the result so that \cpp{expect<expect<pair<int,int>>>} is unwrapped to \cpp{expect<pair<int,int>>}. 
If it is valid it calls to the lambda and unwraps the result. It it is invalid, it returns an \cpp{expect<pair<int,int>>} with the obtained error.
Line 5: returns an expected that combines first and the success of the call to  \cpp{getInt(s, e)}.

Next follows the \cpp{pair_fold} function
...
When \cpp{F()} returns \cpp{expected<void>}, the following function will return an \cpp{expected<T>} . It contents depends on the result of the call to \cpp{f}. if the expected result is valid it return an expected with the first argument, otherwise, an expected with the error of the result of the fall to \cpp{f}.  

When \cpp{F()} returns \cpp{expected<U>}, the following function will return an \cpp{expected<pair<T,U>>} . It contents depends on the result of the call to \cpp{f}. if the expected result is valid it return an expected with a pair of its first argument and the value of  the result of the call to \cpp{f} , otherwise, an expected with the error of the result of the fall to \cpp{f}.  
