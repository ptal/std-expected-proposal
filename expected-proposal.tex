...
The \cpp{fmap} members calls the continuation provided if expected contains a value, otherwise it forwards the error to the callee. Using lambda function might clutter the code, so here an example using functor:
...
This is nice in the sense that whenever there is an error we get a specific error result. However the ``clean code'' characteristic introduced in section \ref{motiv-scope} is not well-respected as the readability doesn't differ much from the ``C return code''. We can rewrite this using a functional style using the member function \cpp{mbind}\footnote{\cpp{mbind} stands for ``monadic bind''.}:as follows:
...
However, there is yet a need to apply a function when all the expected are valued. The free function \cpp{fmap()} takes a variadic continuation and a variadic number of expected parameters. The type and number of the parameters must be compatible with the continuation arguments.
...
\hline
\textbf{visitation} & no & then/mbind/catch_error  & then / (next/recover see \cite{MoreAsync}) \\
\hline
