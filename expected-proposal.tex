...
\noindent
But \cpp{expected<T>} delivers cleaner code when used in a functional style:
...
expected<int> f2(int i, int j, int k)
{
  ...
  if (!eq1.valid()) return eq1;

  auto eq2 = safe_divide(j, k);
  if (!eq2.valid()) return eq2;

  return *q1 + *q2;
}
\end{lstlisting}

This is nice in the sense that whenever there is an error we get a specific error result. Now let's rewrite this using a possible C++ language extension:
