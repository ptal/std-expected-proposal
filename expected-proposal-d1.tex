...
\newenvironment{wordingTextItem}[1]{\wordingItem{#1}\noindent\begin{adjustwidth}{12pt}{}}{\end{adjustwidth}}
...
\subsection{Getting an int from an stream}
%%%%%%%%%%%%%%%%%%%%%
...
In addition to manage with the Iterators, the function changes the error and the int value itself. Let me start by defining the function as if the functions throw an exception when there is an error and make use of an hypothetic \cpp{istream_range} class that would encapsulate the iterators. 
...
When exceptions are not desirable, we could use \cpp{expected<>} as follows

\begin{lstlisting}
  expected<int> getInt(istream_range& r) {
    int result;
    //...
    if (r.empty())  
      return make_unexpected(EmptyStream()); // (1)
    // ..
    if (nonDigit)  
      return make_unexpected(NotANumber()); // (1)
    //...
    return result; // (2)
  }
\end{lstlisting}

(1) uses un implicit conversion from \cpp{unexpected_type<E>} to \cpp{expected<T,E>}. 

(2) uses un implicit conversion from \cpp{T} to \cpp{expected<T,E>}.

It is easy to signal when the function fails giving as much information as needed about why the function failed as well. Just replacing \cpp{throw exception} by \cpp{return make_unexpected(exception)}. 

The caller can obtain the stored value by calling to the member \cpp{value()} function 

\begin{lstlisting}
  // Caller
  std::string s = istream_range(std::cin);
  auto x = getInt(r).value(); // throw on error
\end{lstlisting}

The caller can also store the expected value and check if a value is stored using the member function \cpp{valid()}  
  
\begin{lstlisting}
  auto y = getInt(r); // won't throw
  if (! y.valid()) 
    int i = y.value(); // just "re"throw the stored exception
\end{lstlisting}


The caller can also store the expected value and check later if a value is stored using the member function \cpp{valid()} . When the user knows that there is a value it can use the narrowed contact \cpp{operator*()}.
...
However, it's not easy to write code which detects which of the two conditions occurred. For instance, we could want to fallback to 0 when the input is empty but if the contents of the stream is not an integer we want to preserve the  error. Let's try to write this using our \cpp{getIntOrZero} function, first using exceptions

\begin{lstlisting}
  int getIntOrZero(istream_range& r) 
  {
    try {
      return getInt();
    } catch(EmptyStream& ex) {
      return 0;
    }
  }
\end{lstlisting}

and then using \cpp{expected<>}
...
While this could be useful this doesn't allows to retrieve all the information. In order to retrieve the unexpected value, the caller could use the member function \cpp{catch_exception()}. This function accepts a function having as parameter the template parameter, but this function will be called only when the stored unexpected value matches the template parameter     

\begin{lstlisting}
  expected<int> getIntOrZero(istream_range& r) 
  {
    auto r = getInt();  // won't throw
    r = r.catch_exception<NotANumber>([](auto& ex) // (1)
...
This is quite close to the exception based code. The liability is that these functions are going to be tedious to use when the error must be just forwarded. For instance, the exception based function that duplicates the first int on an stream:

\begin{lstlisting}
int f1(istream_range& r)
{
  return 2 * getInt(r);
}
\end{lstlisting}

becomes using \cpp{expected<int>} 

\begin{lstlisting}
expected<int> f1(istream_range& r)
{
  auto e = getInt(r);
  if (! e.valid()) return e.get_unexpected(); // (1)
  return 2 * *e;  // (2) 
}
\end{lstlisting}

(1) When \cpp{getInt(r)} fails, \cpp{f1} forwards the unexpected value.

(2) Here we know that \cpp{e} has a valid value and so we can get the value without any trouble.

For a so simple function, the code is quite cumbersome. In addition, the return type \cpp{expected<int>} is just not an \cpp{int}, so it's going to be hard to compose functions which return these \cpp{expected<>} values with functions that accept the underlying type. 

Lets continue with this very simple function that returns the sum of the two first ints on a stream when get∞int is exception based:

\begin{lstlisting}
int sumFirstAndSecond(istream_range& r)
{
  auto s1 = getInt(r);
  auto s2 = getInt(r);
  return s1 + s2;
...
Now let's write this code using an \cpp{expected<int>} class. We get this:

\begin{lstlisting}
expected<int> sumFirstAndSecond(istream_range& r)
{
  auto es1 = getInt(r);
  if (!es1.valid()) return es1.get_unexpected();
  auto s1 = *es1;

  auto es2 = getInt(r);
  if (!es2.valid()) return es2.get_unexpected();
  auto s2 = *es2;
...
This is nice in the sense that whenever there is an error (as we have defined it) we get a specific error result. 

The function can be rewritten using a continuation passing style as follows:

\begin{lstlisting}
 expected<int> sumFirstAndSecond3(istream_range& r)
{
  return getInt(r).next([&](auto s1) {
      return getInt(r).next([=](int s2)  {
        return s1+s2;
      });
  });
}
...
The member function next calls the continuation only if the object is valid, otherwise it propagates the error, it returns an expected of the same type as the result of the function but containing the unexpected value.

Alternatively we can also compose all the expected values using the free function \cpp{next}.

\begin{lstlisting}
expected<int> sumFirstAndSecond4(istream_range& r)
{
  return next([](int s1, int s2){
        return s1+s2;
      }, getInt(r), getInt(r));
}
\end{lstlisting}

This \cpp{next} function will call to the continuation only if all the expected parameters are valid. Otherwise it returns an expected of the same type as the result of the function but containing the first unexpected value.

Having a rich functor library could make this even more readable
...
Now let's rewrite this using a possible C++ language extension: adding a \cpp{expect} statement, something similar to the \cpp{await} extension for futures.   

The grammar could be

\begin{lstlisting}
expect-expression ::= expect-initialization ':' expect-expression-or-expression

expect-expression-or-expression ::= expect-expression |†expression

expect-initialization ::= type var '=' 'expect' expression
\end{lstlisting}

The meaning of \cpp{expect-expression}  is given by a transformation \cpp{[[ ]]}

\begin{lstlisting}
[[expect-expression]] =
  next(expression,[&](type var) {
    return [[expect-expression-or-expression]]
  });
\end{lstlisting}
...
\begin{lstlisting}
expected<int> sumFirstAndSecond2(istream_range& r)
{
  return (
  	auto s1 = expect getInt(r) :
  	auto s2 = expect getInt(r) :
   	s1 + s2
  );
}
\end{lstlisting}
...
expected<int> sumFirstAndSecond2(istream_range& r)
{
  return next(getInt(r),[&r](auto s1) {
    return next(getInt(r),next([&r](auto s2) {
      return s1 + s2;
    });
  }); 
}
...
This would give the exact same results as the previous version. However, the function \cpp{sumFirstAndSecond2} is much simpler and clearer than \cpp{sumFirstAndSecond} because it doesn't have to explicitly handle any of the error cases. When an error case occurs, it is returned as the result of the function, but if not, the correct result of a subexpression is bound to a name (\cpp{s1} or \cpp{s2}), and that result can be used in later parts of the computation. The code is a lot simpler to write. The more complicated the error-handling function, the more important this will be.

But, the standard doesn't have this \cpp{except} expression yet.
Waiting for a expect-statement the user could define some macros (see \cite{monad_do_poc} and define \cpp{sumFirstAndSecond} as

\begin{lstlisting}
expected<int> sumFirstAndSecond6(istream_range& r)
{
    return DO (
        ( s1, getInt(r) )
        ( s2, getInt(r) )
...
the previous function could be defined as

\begin{lstlisting}
 expected<int> sumFirstAndSecond7(istream_range& r)
{
  return getInt(r) + getInt(r);
}
\end{lstlisting}

which is the cleaner we can get at the expenses of defining the \cpp{operator+}.

In the case of expected, optional, that is, monads that are synchronous and have only one value stored, the following macro 
...
\subsection{Using a specific error type}
%%%%%%%%%%%%%%%%%%%

The preceding examples make use of the defaulted template parameter \cpp{Error} that defaults to \cpp{std::exception_ptr}. However, in some cases, the user could prefer to use a specific \cpp{Error} class.

\begin{lstlisting}
  expected<int, errc> getInt(istream_range& r);
    int result;
    //...
    if (r.empty())  
      return make_unexpected(errc::empty_stream());
    // ..
    if (nonDigit)  
      return make_unexpected(errc::not_a_number());
    //...
    return result;
  }
\end{lstlisting}
...
The member function \cpp{recover} could be used with a continuations passing style to try to recover from an error.

\begin{lstlisting}
  expected<int,errc> getIntOrZero(istream_range& r);
    return getInt().recover([](auto err) -> expected<int> {
      if ( err == errc::empty_stream ) {
        return 0;
      } else {
...
The continuation function is called only if the expected value is not valid.

As \cpp{expected<int, errc>} doesn't store a exception, when it doesn't contains a value the member function \cpp{value} throws a \cpp{bad_expected_access} exception wrapping the stored error. 

\subsection{Getting an int range from an stream}
%%%%%%%%%%%%%%%%%%%%%%%%

The next use case show how expected scales and compose. 

Given a function that gets an int \cpp{getInt} and one that matches a string \cpp{matchesString}, 
...
how to define a function that parses an int range"<int>..<int>" \cpp{getIntRange}
...
Clearly this is quite clean. With \cpp{expected<>} the signature of \cpp{getInt} ,  \cpp{matchesString} and \cpp{getIntervalInt} using \cpp{expected}, could be 
...
But, how to define \cpp{getIntRange}. Let me start by the basic solution
...
expected<pair<int, int>, errc> getIntervalInt(istream_range& r) {
  return getInt(r)
   .next( [&r](int f){ return matchedString("..", r).next([f]() { return f; }); }
  ).next( [&r](int f){ return getInt(r).next([f](int l) { return std::make_pair(f,l); }); }
  );
}
\end{lstlisting}
...
expected<pair<int, int>, errc> getIntervalInt(istream_range& r) {
  return getInt(r)
   .next( [&r](int f){ return matchedString("..", r).next( identity(f)); }
  ).next( [&r](int f){ return getInt(r).next( pair_1st(f)); }
  );
}
\end{lstlisting}

Having a good functor library could help again, but with the not yet proposed \cpp{expect} operator this would be a little bit simpler
...
An object of type \cpp{E} is not convertible to an unexpected object of type \cpp{expected<T,E>}:

The proposed interface uses a special tag \cpp{unexpect} and a special non-member \cpp{make_unexpected} function to indicate an unexpected state for \cpp{expected<T,E>}. It is used for construction, assignment and relational operations. This might rise a couple of objections. First, this duplication is not strictly necessary because you can achieve the same effect by using the \cpp{unexpect} tagged forwarding constructor:
...
While \cpp{make_unexpected(E)} has been chosen because it clearly indicates that we are interested in creating an unexpected \cpp{expected<T,E>} (of unspecified type \cpp{T}), it could be used also to make a ready future with a specific exception, but this is outside the scope of this proposal.
...
inline namespace fundamentals_v2 {
  ...
  template <> expected<void> make_expected(); 
...
A program that necessitates the instantiation of template \cpp{expected<T,E>} with \cpp{T} for a reference type\, or for possibly cv-qualified types \cpp{in_place_t}, \cpp{unexpect_t} or \cpp{unexpected_type<E>} is ill-formed.

\wordingSubSec{Definitions}{expected.defs}
%%%%%%%%%%%%%%%%%%%%%%

An instance of \cpp{expected<T,E>} is said to be valued if it contains an value of type \cpp{T}.
...
\begin{lstlisting}
  ...
  public:
    ...
    template <typename F>
      auto next(F&& func) const -> expected<decltype(func(val)), E>;
    template <typename F>
      expected<T,E> recover(F&& f);
    template <typename F>
      auto then(F&& func) const -> expected<decltype(func(*this)), E>;
...
expected<T, E> expected<T, E>::catch_exception(F&& func) const;
\end{lstlisting}
\begin{wordingPara}
\begin{wordingTextItem}{Effects}
...
\end{wordingTextItem}
\begin{wordingTextItem}{Returns}
if \cpp{has_exception<Ex>()} returns the result of the call continuation function \cpp{fuct} possibly wrapped on a \cpp{expected<T,E>}, 
...
\begin{lstlisting}[language=C++][xleftmargin=0pt]
template <class Ex,class F>
auto expected<T, E>::then(F&& func) const -> unwrap_nested_expected_t<expected<decltype(func(val)), E>>;
\end{lstlisting}
\begin{wordingPara}
\begin{wordingTextItem}{Returns}
returns \cpp{unwrap(expected<decltype(func(val)), E>(funct(*this)))}, 
\end{wordingTextItem}
\end{wordingPara}
...
\begin{lstlisting}[language=C++][xleftmargin=0pt]
template <class Ex,class F>
auto expected<T, E>::next(F&& func) const -> unwrap_nested_expected_t<expected<decltype(func(val)), E>>;
\end{lstlisting}
\begin{wordingPara}
\begin{wordingTextItem}{Returns}
if \cpp{bool(*this)} returns \cpp{unwrap(expected<decltype(func(val)), E>(funct(**this)))}, 
...
\begin{lstlisting}[language=C++][xleftmargin=0pt]
template <class Ex,class F>
expected<T, E> expected<T, E>::recover(F&& func) const;
\end{lstlisting}
\begin{wordingPara}
\begin{wordingTextItem}{Returns}
if \cpp{! (*this)} returns \cpp{unwrap(expected<decltype(func(val)), E>(funct(**this)))}, 
if \cpp{! *this} returns the result of the call continuation function \cpp{fuct} possibly wrapped on a \cpp{expected<T,E>}, 
otherwise, returns \cpp{*this}.
\end{wordingTextItem}
\end{wordingPara}
...
\begin{lstlisting}
  ...
  public:
    ...
    template <typename F>
      auto next(F&& func) const -> expected<decltype(func()), E>;
    template <typename F>
...
The class \cpp{expected_default_constructed} defines the type of objects thrown as exceptions to report the situation where an attempt is made to access the \cpp{expected<T>::error()}/\cpp{expected<T>::get_unexpected()} value of a unexpected expected object that has no exception stored.
...
Thanks to Fernando Cacciola, Andrzej Krzemie≈Ñski and every one that has contributed to the wording and the rationalle of N3793 \cite{OptionalRev5}.
